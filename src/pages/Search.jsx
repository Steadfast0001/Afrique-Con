...
export default function Search() {
  ...
  const getResults = () => {
    ...
    return schedules
      .filter(s => matchingRouteIds.includes(s.routeId))
      .map(schedule => {
        const route = routes.find(r => r.id === schedule.routeId);
        const bus = buses.find(b => b.id === schedule.busId);
        const scheduleBookings = bookings.filter(b => b.scheduleId === schedule.id && b.checkInStatus !== 'Cancelled');
        const bookedCount = scheduleBookings.reduce((sum, b) => sum + b.seats.length, 0);
        const seatsLeft = bus ? bus.capacity - bookedCount : 0;
        return { ...schedule, route, bus, seatsLeft };
      });
  };
      ...
      <div className="bg-gradient-to-b from-stone-900 to-stone-800 border-b border-stone-700 py-5 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <form onSubmit={handleSearch} className="bg-white rounded-xl p-4 flex flex-col sm:flex-row gap-3 items-end shadow-lg">
            ...
            <button type="submit"
              className="bg-amber-500 hover:bg-amber-400 text-gray-900 font-bold px-7 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors whitespace-nowrap flex-shrink-0">
              ...
              <div key={trip.id} className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-4 hover:shadow-md transition-shadow">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <div className="text-center border border-amber-200 rounded-lg px-4 py-2 bg-amber-50/50">
                      <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">GOLD VIP+</p>
                      <p className="text-lg font-black text-amber-600">{((trip.route?.price || 0) * 2.5).toLocaleString()}</p>
                      <p className="text-[10px] text-amber-500 font-medium">FCFA</p>
                    </div>
                  </div>
                </div>
