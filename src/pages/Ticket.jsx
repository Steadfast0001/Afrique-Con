...
import { Ticket as TicketIcon, Printer, CheckCircle, ShieldCheck, Home } from 'lucide-react';
  ...
  if (!booking || !schedule || !route || !bus) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <button onClick={() => navigate('/')} className="mt-4 text-amber-500 hover:text-amber-600 font-bold">{t('ticket.goHome')}</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="text-center mb-8 animate-fade-in no-print">
        ...
        <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2">{t('ticket.confirmed')}</h2>
      ...
      >
        ...
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-5 text-stone-950 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-extrabold text-lg tracking-wider">{t('ticket.boardingPass')}</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] font-bold uppercase tracking-wider block opacity-70">{t('ticket.bookingRef')}</span>
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex items-center justify-between pb-6 border-b border-gray-200/80">
            <div>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.departureStation')}</span>
            ...
            <div className="text-right">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.arrivalStation')}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-6 border-b border-gray-200/80">
            <div className="space-y-4">
              <div>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.passengerName')}</span>
                <span className="text-gray-900 font-bold text-base">{booking.passengerName}</span>
              </div>
              <div>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.contactEmail')}</span>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.seatsAssigned')}</span>
                  <span className="text-amber-600 font-extrabold text-base tracking-wider">{booking.seats.join(', ')}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.busClass')}</span>
                  <span className="text-gray-700 font-semibold text-sm">{bus.type}</span>
                </div>
              </div>
              {booking.passportNumber && (
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.travelDoc')}</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pb-6 border-b border-gray-200/80 text-sm">
            <div>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.date')}</span>
              <span className="text-gray-900 font-bold">{schedule.departureDate}</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.departureTime')}</span>
              <span className="text-gray-900 font-bold text-amber-500">{schedule.departureTime}</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.gatePlatform')}</span>
              <span className="text-gray-900 font-bold">Platform 3</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{t('ticket.boardingStatus')}</span>
              <span className="text-emerald-600 font-bold">{t('ticket.paidConfirmed')}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pt-2">
            ...
            {/* Fare detail summary */}
            <div className="text-right">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-0.5">{t('ticket.totalFare')}</span>
              <span className="text-2xl font-extrabold text-amber-600">{booking.totalAmount.toLocaleString()} FCFA</span>
              <p className="text-[10px] text-gray-500 font-medium">{t('ticket.settledVia')} {booking.paymentMethod}</p>
            </div>
          </div>
      ...
      <div className="mt-8 flex justify-between items-center no-print">
        ...
        >
          ...
          <span>{t('ticket.printTicket')}</span>
        </button>
      </div>
